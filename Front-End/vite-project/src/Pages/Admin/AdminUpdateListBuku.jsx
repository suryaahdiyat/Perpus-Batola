...
export default function AdminUpdateListBuku() {

  const judulRef = useRef("")
  const tahunRef = useRef(null)
  const pengarangRef = useRef("")

  const location = useLocation()
  const navigate = useNavigate()
  const idBuku = location.pathname.split('/')[4]
  ...
  useEffect(()=> {
    const fetchingData = async() => {
      const result = await axios.get("http://localhost:3030/buku/"+idBuku)
      // console.log(result.data[0])
      const bukuBaru = {
        judul:result.data[0].judul,
        tahun:result.data[0].tahun,
        pengarang:result.data[0].pengarang
      }
  ...
  const handleClick = async(e) =>{
    ...
    const bukuBaru = {
      ...
      "pengarang":pengarangRef.current.value
    }
    try{
      await axios.put("http://localhost:3030/buku/"+idBuku, bukuBaru)
      navigate("/admin/list-buku")
    }catch(err){
      console.log(err)
    }
  }
        ...
        <div className='w-full overflow-hidden flex flex-col justify-between'>
          <div>
            <div className='bg-sky-700 h-28 p-3 flex items-center gap-4'>
              <div className='ml-3 w-14 h-14  flex items-center justify-center text-2xl text-white font-bold'>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-13 h-13">
  ...
  <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0 0 13.5 3h-6a2.25 2.25 0 0 0-2.25 2.25v13.5A2.25 2.25 0 0 0 7.5 21h6a2.25 2.25 0 0 0 2.25-2.25V15M12 9l-3 3m0 0 3 3m-3-3h12.75" />
        ...
        <div className='w-3/4 bg-slate-950 text-white p-10 flex-col'>
            <FormUpdate buku={buku} judulRef={judulRef} tahunRef={tahunRef} pengarangRef={pengarangRef} handleClick={handleClick} />
        </div>
    </div>
  )
}
