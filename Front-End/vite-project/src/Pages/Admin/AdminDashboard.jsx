...
export default function AdminDashboard() {
  const [sPembaca, setSPembaca] = useState(0)
  const [sBuku, setSBuku] = useState(0)

  useEffect(() => {
    const fetchingDataPembaca = async() => {
      try{
        const result = await axios.get("http://localhost:3030/users-count")
        setSPembaca(result.data.jumlah_users)
      }catch(err){
        console.log(err)
      }
    }

    const fetchingDataBuku = async() => {
      try{
        const result = await axios.get("http://localhost:3030/buku-count")
        setSBuku(result.data.jumlah_buku)
      }catch(err){
        console.log(err)
      }
    }
  ...
  return (
    ...
    <path d="M12 7.875a1.125 1.125 0 1 0 0-2.25 1.125 1.125 0 0 0 0 2.25Z" />
            ...
            <Link to={'/admin/dashboard'} className='bg-slate-800 p-5 flex items-center gap-4 border-b-2 border-slate-600'>
  ...
  <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0 0 13.5 3h-6a2.25 2.25 0 0 0-2.25 2.25v13.5A2.25 2.25 0 0 0 7.5 21h6a2.25 2.25 0 0 0 2.25-2.25V15M12 9l-3 3m0 0 3 3m-3-3h12.75" />
      ...
      <div className='w-3/4 overflow-auto'>
        <div className='h-full w-full bg-slate-950 p-10'>
          <h1 className='text-white text-3xl font-bold mb-10'>Data Saat Ini di Perpustakaan Barito Kuala</h1>
          <div className='text-white flex justify-between w-full gap-4'>
            <div className='w-6/12 h-40 bg-slate-700 rounded-xl overflow-hidden'>
              <h1 className='w-full text-center bg-emerald-600 py-3 text-lg'>Jumlah Semua Pembaca</h1>
              <div className='text-3xl flex justify-center items-center w-full h-4/6'>{sPembaca}</div>
            </div>
            <div className='w-6/12 h-40 bg-slate-700 rounded-xl overflow-hidden'>
              <h1 className='w-full text-center bg-red-500 py-3 text-lg'>Jumlah Semua Buku</h1>
              <div className='text-3xl flex justify-center items-center w-full h-4/6'>{sBuku}</div>
            </div>
          </div>
        </div>
      </div>
